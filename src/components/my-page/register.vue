<template>
    <div class="wrapper">
        ...
        <div class="mui-input-row mui-password myclass">
            <form action="">
                <input :type=" inputIsPassword ? 'password' : 'text' " class="mui-input-password" placeholder="请再次输入密码" autocomplete="off" v-model="confirmPass">
                <span class="mui-icon mui-icon-eye" @click="changeInputType" :style=" 'color:'+ (inputIsPassword ? 'grey' : 'blue') "></span>
            </form>
        </div>
        <div class="captcha">
        ...
        <button type="button" class="mui-btn mui-btn-success mui-btn-block" @click="checkInput">注册</button>
        <router-link to="/my/login">
            <span class="turn">登录</span>
        </router-link>
...
export default {
    ...
    methods: {
        ...
        register() {
            this.$http.post( this.getHost() + '/register', {  username: this.username.trim(), password: this.password.trim() }, 
            { emulateJSON: true } ).then(
                res => {
                    if ( res.body.res_code == 1 ) {
                        this.alertText = res.body.message
        ...
        checkInput() {
            if ( this.username.length == 0 || this.password.length == 0 || this.confirmPass == 0 ) {
                console.log( this.username.length, this.password.length, this.confirmPass.length )
                this.alertText = '用户名或密码或确认密码不能为空！'
                this.inputError = true
                return 
            }
            else if( this.password.trim() !== this.confirmPass.trim()) {
                this.alertText = '两次密码输入不正确！'
                this.inputError = true
                return
            }
